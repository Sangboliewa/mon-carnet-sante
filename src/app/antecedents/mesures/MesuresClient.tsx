        ...
        {[...filtered].reverse().map((item) => {
          ...
          return (
            <div key={item.id} className="card flex justify-between items-center">
              <div>
                <p className="font-semibold text-gray-900 text-sm">{formatVal(item)}</p>
                <p className="text-xs text-gray-400">{new Date(item.measured_at).toLocaleString("fr-FR")}</p>
                {item.context && <p className="text-xs text-gray-500">{CONTEXTS.find(c => c.value === item.context)?.label}</p>}
              </div>
              <div className="flex flex-col items-end gap-1">
                {s !== "unknown" && (
                  <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${s === "alert" ? "bg-red-100 text-red-700" : "bg-green-100 text-green-700"}`}>
                    {s === "alert" ? "⚠" : "✓"}
                  </span>
                )}
                <button onClick={() => handleDelete(item.id)} disabled={deletingId === item.id} className="text-red-400 text-xs">
